...
sealed trait FunctionRoleSource {
   ...
   */
  def asTFValue(function: ScalambdaFunction): TValue
}
...
object FunctionRoleSource {
  ...
  case object FromVariable extends FunctionRoleSource {
    override def asTFValue(scalambdaFunction: ScalambdaFunction): TValue = TVariableRef(variableName(scalambdaFunction))
    def variableName(scalambdaFunction: ScalambdaFunction): String = s"${scalambdaFunction.terraformLambdaResourceName}_role_arn"
  }
  ...
  case class StaticArn(roleArn: String) extends FunctionRoleSource {
     ...
     * @param function to infer name from
     */
    override def asTFValue(function: ScalambdaFunction): TValue = TString(roleArn)
  }
