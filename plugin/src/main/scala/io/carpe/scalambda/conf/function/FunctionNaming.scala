...
sealed trait FunctionNaming {
  def asTValue(function: ScalambdaFunction): TValue
}
...
object FunctionNaming {
  ...
  case object WorkspaceBased extends FunctionNaming {
    override def asTValue(function: ScalambdaFunction): TValue = {
  ...
  case class Static(name: String) extends FunctionNaming {
    override def asTValue(function: ScalambdaFunction): TValue = TString(name)
  }
}